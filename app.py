...
# BIBLE_JSON_FILE = os.path.join(BIBLE_CACHE_DIR, "bible_krv.json")  # bible_krv.json 경로
# _bible_data_cache = None  # 전체 성경 데이터를 한번만 로딩하기 위한 캐시
# _bible_meta_cache = None  # 메타 데이터 캐시
...
#             _bible_meta_cache = json.load(f)
#     return _bible_meta_cache
#     # if os.path.exists(BIBLE_META_FILE):
#     #     with open(BIBLE_META_FILE, 'r', encoding='utf-8') as f:
#     #         return json.load(f)
#     # return {"books": [], "abbr_to_id": {}}


# def get_bible_chapter(book_id: str, chapter: int) -> dict:
#     """장 전체 절 딕셔너리 반환 {절번호(int): 텍스트}"""
#     global _bible_data_cache
#     if _bible_data_cache is None:
#         if not os.path.exists(BIBLE_JSON_FILE):
#             return {"error": f"{BIBLE_JSON_FILE} 파일이 없습니다."}
...
#     # """장 전체 절 딕셔너리 반환 {절번호(int): 텍스트}. 캐시 우선."""
#     # cache_file = os.path.join(BIBLE_CACHE_DIR, f'{book_id}_{chapter}.json')
#     # if os.path.exists(cache_file):
#     #     with open(cache_file, 'r', encoding='utf-8') as f:
#     #         data = json.load(f)
#     #         return {int(k): v for k, v in data.items()}

#     # url_book = BOOK_URL_MAP.get(book_id, book_id.lower())
#     # url = f'https://getbible.net/v2/kor/{url_book}/{chapter}.json'
#     # try:
#     #     req = urllib.request.Request(url, headers={'User-Agent': 'Mozilla/5.0'})
#     #     with urllib.request.urlopen(req, timeout=10) as resp:
#     #         raw = json.loads(resp.read().decode('utf-8'))
#     #     verses_raw = raw.get('chapter', {})
#     #     result = {}
#     #     for v_num, v_data in verses_raw.items():
#     #         text = v_data.get('verse', '') if isinstance(v_data, dict) else str(v_data)
#     #         result[int(v_num)] = text.strip()
...
# @app.route('/api/bible', methods=['POST'])
# def api_bible():
#     """POST {book_abbr, chapter, verse_start, verse_end}"""
#     data = request.json or {}
#     meta = load_bible_meta()
#     abbr        = data.get('book_abbr', '')
#     chapter     = int(data.get('chapter', 1))
...
#                 if paragraphs:
#                     prs = Presentation(work_path)
#                     set_slide_text(prs.slides[lyrics_idx], paragraphs[0])
#                     prs.save(work_path)
#                     for i, para in enumerate(paragraphs[1:], 1):
#                         insert_after = lyrics_idx + i - 1
#                         new_path = duplicate_slide_zip(work_path, lyrics_idx, insert_after)
#                         os.unlink(work_path); work_path = new_path
#                         prs = Presentation(work_path)
#                         set_slide_text(prs.slides[insert_after + 1], para)
#                         prs.save(work_path)

#         # ── 2. Scripture verses (2절씩) ───────────────────────────────
#         if scripture and not scripture.get('skip'):
#             sc_idx = scripture.get('slide_index')   # 0-based
#             verses = scripture.get('verses', [])     # [{num, text}, ...]
...
#         # ── 3. Hymn insertion (back-to-front) ─────────────────────────
#         active = [s for s in hymn_slots if not s.get('skip') and s.get('hymn_number')]
#         active = sorted(active, key=lambda x: x.get('after_slide_index', 0), reverse=True)
...
# def copy_slide_from_file_zip(src_path, src_slide_index, dst_path, insert_after):
#     """Copy slide from src into dst at position insert_after+1. Returns new temp pptx path."""
#     out_fd, out_path = tempfile.mkstemp(suffix='.pptx')
#     os.close(out_fd)
...
# def set_slide_text(slide, text_content):
#     """가장 큰 텍스트 박스에 텍스트를 교체. 줄바꿈(\n) 지원, 기존 서식 유지."""
#     best_shape, best_size = None, 0
#     for shape in slide.shapes:
#         if shape.has_text_frame:
#             size = shape.width * shape.height
...
#     txBody  = tf._txBody
#     exist_p = txBody.findall(qn('a:p'))
#     tmpl_p  = copy.deepcopy(exist_p[0]) if exist_p else None
#     for p in exist_p:
#         txBody.remove(p)
...
# set_slide_lyrics = set_slide_text
...
def generate():
    ...
    try:
        ...
        # ── 3. 추가 구절 (extra_verses) ───────────────────────────────
        # 역순 처리: 인덱스 큰 것부터 삽입해야 앞쪽 인덱스에 영향 없음.
        # "두 번째 추가구절부터" = 원래 정렬 순서에서 index 1+
        ev_in_order = sorted(
            [ev for ev in extra_verses if ev.get('slide_index') is not None and ev.get('verses')],
            key=lambda x: x['slide_index']
        )
        ...
        for ev_orig_order, ev in enumerate(reversed(ev_in_order)):
            # scripture 처리로 삽입된 슬라이드 수만큼 ev_idx 보정
            # (sc_lyrics_idx보다 뒤에 있는 슬라이드만 영향 받음)
            raw_idx   = ev['slide_index']
            sc_lyrics_idx_val = scripture.get('lyrics_slide_index') if scripture and not scripture.get('skip') else None
            if sc_lyrics_idx_val is not None and raw_idx > sc_lyrics_idx_val:
                ev_idx = raw_idx + sc_inserted
            else:
                ev_idx = raw_idx
            ...
            for i, pair in enumerate(pairs):
                ...
                prs = Presentation(work_path)
                ev_target_slide = prs.slides[ins_after + 1]
                set_slide_text_bibel(ev_target_slide, pair)
                add_chapter_title_text(ev_target_slide, f"{ev_book} {ev_ch}장")

                prs.save(work_path)
        prs = Presentation(work_path)
        delete_slide(prs, ev_idx)
        prs.save(work_path)
